...
export default function Results({
  ...
  const getScoreLevel = (score: number) => {
    if (score >= 80) return {
      level: "Excellent",
      color: "success",
      description: "Your privacy practices are strong"
    };
    if (score >= 60) return {
      level: "Good",
      color: "primary",
      description: "You have solid foundations with room for improvement"
    };
    if (score >= 40) return {
      level: "Needs Improvement",
      color: "warning",
      description: "Several areas require attention"
    };
    return {
      level: "Critical",
      color: "destructive",
      description: "Immediate action required to ensure compliance"
    };
  };
  ...
  const downloadAssessmentCSV = () => {
    const csvData = [['Assessment Results - תיקון 17 Privacy Compliance'], ['Generated on', new Date().toISOString()], ['Overall Score', `${overallScore}%`], ['Assessment Level', scoreLevel.level], [''], ['Question', 'Answer', 'Category', 'Weight', 'Score'], ...Object.entries(answers).map(([question, answer]: [string, any]) => [question, answer.text || 'N/A', answer.category, answer.weight.toString(), `${Math.round(answer.weight / 5 * 100)}%`]), [''], ['Category Scores'], ['Category', 'Score'], ...Object.entries(categoryScores).map(([category, data]: [string, any]) => [category, `${Math.round(data.total / (data.count * 5) * 100)}%`]), [''], ['Recommendations'], ...recommendations.map((rec, index) => [`${index + 1}`, rec])];
    const csvContent = csvData.map(row => Array.isArray(row) ? row.map(field => `"${String(field).replace(/"/g, '""')}"`).join(',') : `"${String(row).replace(/"/g, '""')}"`).join('\n');
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <Card className="p-8 mb-8 shadow-elegant text-center">
          <div className="mb-6">
            ...
            <Badge variant={scoreLevel.color === "success" ? "default" : scoreLevel.color === "warning" ? "secondary" : "destructive"} className="text-lg px-4 py-2">
              {scoreLevel.level}
            </Badge>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          return <Card key={category} className="p-6">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {categoryScore >= 80 ? <CheckCircle className="w-6 h-6 text-success" /> : <AlertTriangle className="w-6 h-6 text-warning" />}
                </div>
        ...
        {/* Recommendations */}
        {recommendations.length > 0 && <Card className="p-8 mb-8">
            <h3 className="text-2xl font-semibold mb-6 flex items-center">
              <AlertTriangle className="w-6 h-6 text-warning mr-3" />
              Priority Recommendations
            </h3>
            <div className="space-y-4">
              {recommendations.map((rec, index) => <div key={index} className="flex items-start space-x-3 p-4 bg-secondary/50 rounded-lg">
        ...
        <Card className="p-8 text-center bg-gradient-subtle">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button onClick={onContact} variant="hero" size="lg" className="text-lg px-8">
              <Mail className="w-5 h-5 mr-2" />
              Contact Our Privacy Experts
            </Button>
            
            <Button onClick={downloadAssessmentCSV} variant="secondary" size="lg" className="text-lg px-8">
          ...
          <p className="text-sm text-muted-foreground mt-4">
            Email: <span className="font-medium">info@gdpr.co.il</span>
          </p>
        </Card>
      </div>
    </section>;
